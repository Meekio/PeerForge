...
export default function VerifyScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Sending verification with email:', emailToUse);
      
      if (uploadMode === 'pdf') {
        response = await api.verifyId(pdfFile!, null, emailToUse);
      } else {
        response = await api.verifyId(frontId!, backId!, emailToUse);
      }
      ...
      if (response.verified) {
        ...
        setStatus('rejected');
        Alert.alert('Verification Failed', 'Please try uploading clearer images');
      }
    } catch (err) {
      setStatus('rejected');
